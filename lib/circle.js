...
export async function captureLeadWithTags(token, communityId, { email, name, tagNames }) {
  // TEMP DEBUG: probe member_tags with each auth scheme to find which authenticates.
  const rawTagProbe = {};
  const schemes = { bearer: `Bearer ${token}`, token: `Token ${token}`, raw: token };
  for (const [k, v] of Object.entries(schemes)) {
    try {
      const p = await fetch(`${V1}/member_tags?per_page=3`, {
        headers: { Authorization: v, "Content-Type": "application/json" },
      });
      rawTagProbe[k] = { status: p.status, sample: (await p.text()).slice(0, 140) };
    } catch (e) {
      rawTagProbe[k] = { error: String(e) };
    }
  }
